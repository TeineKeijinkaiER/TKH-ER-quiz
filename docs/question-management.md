...
> **総合ワークフローは `docs/data-maintenance-workflow.md` を参照。**
> 文字化け対策、英訳依頼、医学的検証、AI 引き継ぎなど運用全般はそちらにまとめてある。
> このドキュメントは **JSON 仕様の詳細** に特化したリファレンス。
...
- `selectCount`: 選択数ルール。単一選択は `1`、N個選択は `N`、個数指定なしの「すべて選べ」は必ず `0`（後述）
...
- `answer` は正答インデックスの配列
- **`selectCount` は必ず `0` と書く**。削除しない
- `selectCount: 0` が「個数指定なしで、正しいものをすべて選べ」の合図
